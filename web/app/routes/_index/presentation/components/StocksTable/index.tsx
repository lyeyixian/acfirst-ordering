...
export function StocksTable({ stocks }: { stocks: Stock[] }) {
  ...
  useEffect(() => {
    ...
    stocks.map((data) => {
      rows.push({
        ...
        'Price per Unit (MYR)': data.pricePerUnit,
      })
    })
    setRowData(rows)
  }, [stocks])
  ...
  const onSelectionChanged = () => {
    const selectedRows: StockRowData[] = gridRef.current?.api
      .getSelectedNodes()
    ...
    selectedRows.forEach((item) => {
      const cartItem: CartItem = {
        currentQuantity:
          cartItems
            .filter((existingItem) => isSameStockItem(existingItem, item))
            .pop()?.currentQuantity ?? 1, //Set 1 to be default selected quantity
        stock: {
  ...
  const isSameStockItem = (item1: CartItem, item2: StockRowData) => {
    return (
      item1.stock.itemCode === item2['Item Code'] &&
                ...
                >
                  <Flex justify={'space-between'}>
                    ...
                    <Flex>
                      <div style={{ flex: 1 }}>
                        <Group>
                          <Text size="sm" mr="md" fw={500}>
                            {' '}
                            Cost:{' '}
                          </Text>
                          <Text>
                            {item.currentQuantity * item.stock.pricePerUnit}
                          </Text>
                        </Group>
                        <Text size="xs">
                          {' '}
            ...
            <Container className={classes.footerContainer} mt="md">
              <Flex justify={'right'}>
                ...
                <Button
                  ...
                  variant="filled"
                  mx="md"
                  disabled={zeroQuantityItemExist()}
                  onClick={close}
                >
              ...
              {zeroQuantityItemExist() ? (
                <Text
                  mx={'md'}
                  mt={'md'}
                  className={classes.zeroQuantityText}
                  size="sm"
                  c={'red'}
                >
